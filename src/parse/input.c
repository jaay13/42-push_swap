...
/*   Updated: 2026/06/08 15:52:29 by jakoch           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	parse_input(int argc, char **argv, t_config *config)
{
	...
	if (!parse_numbers(argc, argv, config->num_start)) 		// Checks parse_numbers and handles error incase it returns NULL on error
	{
		write(2, "Error\n", 6);
		exit(1);
	}
